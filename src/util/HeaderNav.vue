<template>
    <!-- mx-auto lg:max-w-4xl xl:max-w-6xl 2xl:max-w-7xl -->
    <div class="hidden lg:block mx-auto lg:max-w-4xl xl:max-w-6xl 2xl:max-w-7xl">
        <div class="h-[90px] flex items-center justify-between relative w-full">
            <div >
                <img @click="goToHome" :src="store.state.clientLogo" alt="IIFL-Logo" class="cursor-pointer">
            </div>
            <ul class="flex xl:space-x-16 items-center h-full">
                <li class="text-[14px] cursor-pointer h-full flex items-center justify-center w-[110px] relative select-none"
                    v-for="(record, id) in tabs" :key=id :class="{ 'active_head' : record.path === $route.path , '!cursor-not-allowed' : record?.disabled}" @click="navigateHead(record)">
                    <!-- <RouterLink :to="record.path" class="my-auto flex items-center justify-center h-full w-full ">
                        <span class="inherit-font w-auto">{{ record.name }}</span>
                    </RouterLink> -->
                    <span class="inherit-font w-auto">{{ record.name }}</span>
                </li>
                <Menu as="div" class="relative inline-block text-left pl-[100px]">
                    <div>
                        <MenuButton>
                            <!-- Login / Sign Up -->
                            <div class="flex space-x-2">
                                <img src="./../assets/icons/navIcon.svg" alt="profile">
                                <ChevronDownIcon class="-mr-1 h-5 w-5 text-[#515151] my-auto" aria-hidden="true" />
                            </div>
                        </MenuButton>
                    </div>
                    <transition enter-active-class="transition ease-out duration-100"
                        ...
                        <MenuItems
                            class="absolute right-0 z-10 mt-2 w-[100px] origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                            <div class="py-1">
                                <MenuItem v-slot="{ active }">
                                <a class="cursor-pointer" @click="logOut()"
                                    :class="[active ? '' : 'text-gray-700', 'block px-4 py-4 text-sm bottom text-center']">
                                    Log Out
                                    <!-- <span class="block border-b mx-auto py-2"></span> -->
                                </a>
                                </MenuItem>
                                <!-- <MenuItem v-slot="{ active }">
                            <a href="#" @click="navigateAsPartner()"
                                :class="[active ? '' : 'text-gray-700', 'block px-4 py-2 text-sm ']">
                                As Partner
                            </a>
                            </MenuItem> -->
                            </div>
                ...
                <div class="fixed inset-0 flex">
                    <TransitionChild as="template" enter="transition ease-in-out duration-300 transform"
                        ...
                        <DialogPanel class="relative mr-18 flex w-full max-w-xs flex-1">
                            ...
                            <div class="flex grow flex-col gap-y-[30px] overflow-y-auto bg-white ">
                                <div class="flex justify-between  shrink-0 items-center px-6 ">
                                    <img class="h-[23px] w-[163px] mt-[30px]" src="../assets/img/login/Logo.svg"
                                        alt="Your Company" />
...
<path d="M0.319263 0.320137C0.420471 0.218925 0.540624 0.138639 0.672864 0.0838632C0.805103 0.0290874 0.946838 0.000894218 1.08997 0.000894218C1.23311 0.000894218 1.37485 0.0290874 1.50709 0.0838632C1.63932 0.138639 1.75948 0.218925 1.86069 0.320137L6.99812 5.45724L12.1362 0.320137C12.2374 0.218761 12.3576 0.138312 12.4899 0.0833831C12.6222 0.028454 12.764 0.000120862 12.9073 3.85827e-07C13.0505 -0.00012009 13.1924 0.0279748 13.3248 0.0826813C13.4572 0.137388 13.5775 0.217634 13.6789 0.318839C13.7803 0.420044 13.8608 0.540226 13.9157 0.672521C13.9706 0.804817 13.999 0.946637 13.9991 1.08988C13.9992 1.23313 13.9711 1.375 13.9164 1.50738C13.8617 1.63977 13.7814 1.76009 13.6802 1.86146L8.53955 6.99857L13.6802 12.1363C13.885 12.3411 14 12.6187 14 12.9083C14 13.1978 13.885 13.4755 13.6802 13.6802C13.4755 13.885 13.1978 14 12.9082 14C12.6187 14 12.341 13.885 12.1362 13.6802L6.99812 8.5399L1.86069 13.6802C1.75948 13.7814 1.63932 13.8617 1.50708 13.9165C1.37484 13.9713 1.23311 13.9995 1.08997 13.9995C0.94684 13.9995 0.805108 13.9713 0.672869 13.9165C0.54063 13.8617 0.420474 13.7814 0.319263 13.6802C0.218052 13.579 0.137766 13.4589 0.0829911 13.3267C0.0282159 13.1944 2.37579e-05 13.0527 2.37579e-05 12.9096C2.37579e-05 12.7665 0.0282159 12.6247 0.0829911 12.4925C0.137766 12.3603 0.218052 12.2401 0.319263 12.1389L5.45735 7.00116L0.319263 1.86406C0.218045 1.76286 0.137754 1.64271 0.0829743 1.51048C0.0281949 1.37825 0 1.23652 0 1.09339C0 0.950267 0.0281949 0.808542 0.0829743 0.676311C0.137754 0.544079 0.218045 0.423932 0.319263 0.322731V0.320137Z" fill="#919191"/>
                                ...
                                <nav class="flex flex-1 flex-col px-5">
                                    <ul role="list" class="flex flex-1 flex-col gap-y-7">
                                        <li>
                                            <ul role="list" class="">
                                                <li  @click="navigateTo(item)" @close="sidebarOpen = false"
                                                    :class="[activeTab == item.id  ? 'text-[#F37021] font-bold text-[16px]' : 'text-[#515151] text-[16px]', 'group flex gap-x-3 rounded-none text-sm font-semibold leading-6 border-b', index == 0 ? 'pb-5' : 'py-[30px]']"
                                ...
                                <!-- <div class=" bg-[#3E3E3E] h-auto">
                                    <ul class="px-5 grid grid-cols-1 divide-y  space-y-5 pt-[26px]">
                                        <li class="text-[#FFFFFF] text-[16px] font-medium ">Login as Individual Trader</li>
                                        <li class="text-[#FFFFFF] text-[16px] font-medium py-[30px]"> Login as Partner</li>
                                    </ul>
                                </div> -->
                            </div>
                        </DialogPanel>
        ...
        <div class="lg:pl-72 lg:hidden">
            <div class="sticky top-0 z-40 flex h-[50px] shrink-0 items-center justify-between gap-x-4 border-b border-gray-200 bg-white px-4 shadow-sm sm:gap-x-6 sm:px-6 lg:px-8">
                <div class="flex space-x-[15px]">
                    <button type="button" v-if="location.name != 'login'"  class="sm:block -m-2.5 p-2.5 text-gray-700 lg:hidden" @click="sidebarOpen = true">
                    ...
                    <div class="flex flex-shrink">
                        <img @click="goToHome" class="h-[22px] w-[155px] cursor-pointer" src="../assets/img/login/Logo.svg" alt="IIFL-Logo">
                    </div>
                </div>
                <Menu as="div" class=" relative sm:inline-block text-left pl-[100px]">
                    <div>
                        <MenuButton>
                            <!-- Login / Sign Up -->
                            <div v-if="location.name != 'login'"  class="flex items-center justify-center space-x-1">
                                <img class="h-[22px]" src="./../assets/icons/navIcon.svg" alt="profile">
                                <ChevronDownIcon class="-mr-1 h-4 w-4 text-[#515151] my-auto" aria-hidden="true" />
                            </div>
                        </MenuButton>
                    </div>
                    <transition enter-active-class="transition ease-out duration-100"
                        ...
                        <MenuItems
                            ...
                            <div class="py-1">
                                <MenuItem v-slot="{ active }">
                                <a class="cursor-pointer" @click="logOut()"
                                    :class="[active ? '' : 'text-gray-700', 'block px-2 py-2 text-[12px] bottom text-center']">
                                    Log Out
                                </a>
                                </MenuItem>
                            </div>
...
const tabs = computed(() => store.getters["dashboard/getHeaderTabs"])
...
//     window.open("https://iifl.codifi.in/developer/");
// }

const logOut = ()=>{
    store.commit("setIsLogout", true)
}
...
<style>
.active_head {
    @apply !text-[#F37021] !font-bold after:absolute after:w-full after:h-[2px] after:bg-[#F37021] after:bottom-0 bg-[#FFF5EE];
}
